...
CONFIGS[u'search_keywords_dict'] = {'tour':[u'Suncheon Bay Reeds', u'Seoul Central Mosque', u'Darangee Village', u'Boseong Green Tea Field', u'Jeondong Catholic Church', u'Damyang Metasequoia Road', u'Gwangan Bridge']}
...
def download_image(url, save_dir,search_keywords , MY_N,loaded_urls=None):
    real_url = None
    response = None
    ...
    try:
        ...
        if loaded_urls and real_url in loaded_urls:
            ...
            print 'Try to save image ' + real_url

            output_file = open(save_dir+"/"+search_keywords+str(MY_N)+".txt",'w')
            MY_N=MY_N+1
            data = response.read()
            output_file.write(real_url)
    ...
    return real_url, save_image_name, MY_N
...
#            new_items = _images_get_all_items(raw_html)
            ...
            for url in new_items:
                ...
                real_url, save_name,  MY_N = download_image(url, class_save_dir,keyword, MY_N ,items )
        ...
        print 'len(items)=', len(items)
        if len(items) >= len(search_keywords)*cut_line:         
            break
